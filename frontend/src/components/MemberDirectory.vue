<template>
  <div class="space-y-6">
    ...
    <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      <div v-for="n in 8" :key="n" class="bg-white border border-gray-200 rounded-xl p-5 animate-pulse">
    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      ...
      >
        <!-- Avatar + name + role -->
        <div class="flex items-center gap-3">
          <div class="w-11 h-11 rounded-full bg-khilafat-100 flex items-center justify-center text-khilafat-700 font-bold text-lg shrink-0 select-none">
            {{ u.username[0].toUpperCase() }}
          </div>
          <div class="min-w-0">
            <div class="font-semibold text-gray-900 truncate">{{ u.username }}</div>
            <span
        ...
        <div>
          <div class="flex justify-between text-xs mb-1.5">
            ...
            <span class="font-bold" :class="u.reputationScore >= 40 ? 'text-khilafat-600' : 'text-red-500'">
              {{ u.reputationScore }}
            </span>
          </div>
          <div class="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
            <div
              ...
              :style="{ width: Math.min(100, Math.max(0, u.reputationScore)) + '%' }"
            ></div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between">
          ...
          <span class="text-sm font-bold text-amber-600">{{ toGrams(u.goldBalance) }} g</span>
        </div>
