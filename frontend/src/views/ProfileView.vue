<template>
  <div class="space-y-6">
    ...
    <div v-if="loading" class="space-y-6 animate-pulse">
      <div
        class="bg-white rounded-xl border border-gray-200 shadow-sm p-8 flex gap-6"
      >
    ...
    <div v-else-if="!profile" class="text-center py-24">
      ...
      <p class="text-sm text-gray-400 mt-1">
        The username "{{ $route.params.username }}" does not exist.
      </p>
    </div>
    ...
    <template v-else>
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
        ...
        >
          ...
          <div class="flex-1 text-center sm:text-left">
            <div
              class="flex flex-wrap items-center justify-center sm:justify-start gap-3 mb-1"
            >
            ...
            <div
              v-if="isSelf"
              class="mt-4 inline-flex items-center gap-1.5 px-3 py-1.5 bg-khilafat-50 border border-khilafat-200 text-khilafat-700 text-xs font-semibold rounded-lg"
            >
            ...
            <router-link
              v-else
              :to="{ path: '/send-gold', query: { sendTo: profile.username } }"
      ...
      <div class="grid grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          ...
          >
            <div
              ...
              :style="{
                width:
                  Math.min(100, Math.max(0, profile.reputationScore)) + '%',
              }"
            ></div>
          </div>
          <div
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
          ...
          <div class="text-3xl font-bold text-amber-600">
            {{ toGrams(profile.goldBalance)
            }}<span class="text-base font-medium text-gray-400 ml-1">g</span>
          </div>
          <div class="text-xs text-gray-400 mt-1">
            {{ profile.goldBalance }} mg
          </div>
        </div>

        <!-- Listings count -->
        <div
          class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 col-span-2 lg:col-span-1"
        >
          <div
            class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2"
          >
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        ...
        <div
          v-if="profile.listings.length === 0"
        ...
        >
          ...
          >
            <div class="flex items-start justify-between gap-2 mb-1">
              <div class="font-semibold text-sm text-gray-900 leading-snug">
                {{ p.name }}
              </div>
              <div class="text-sm font-bold text-amber-600 shrink-0">
                {{ toGrams(p.price) }} g
              </div>
            </div>
            <p
...
const isSelf = computed(
  () => profile.value && store.user.username === profile.value.username,
);
