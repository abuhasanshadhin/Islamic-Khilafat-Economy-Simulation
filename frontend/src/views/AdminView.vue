  ...
  <div class="space-y-6">
    ...
    >
      ...
      <div class="flex flex-wrap items-center gap-3 mb-4">
        <select
          v-model="filterType"
          class="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-khilafat-400 bg-white"
        >
        ...
        <table class="w-full text-left text-sm">
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-3 font-medium text-amber-700">
                {{ toGrams(tx.amount) }} g
              </td>
      ...
      <Pagination
        :meta="{
          ...
          rangeStart: Math.max(1, (txPage - 1) * txLimit + 1),
    ...
    >
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <h3 class="text-sm font-semibold text-gray-900 mb-4">
          BaitulMal Gold Reserve Over Time
        </h3>
        <canvas ref="lineChart" height="140"></canvas>
        <p
          v-if="store.baitulHistory.length === 0"
          class="text-xs text-gray-400 text-center mt-2"
        >
          Accumulates as mining events arrive via socket
        </p>
      </div>
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-3 text-amber-700 font-semibold">
                {{ toGrams(u.goldBalance) }} g
              </td>
              <td class="px-4 py-3">
                <span
                  class="font-semibold"
                  :class="
                    u.reputationScore >= 40
                      ? 'text-khilafat-600'
                      : 'text-red-500'
                  "
                  >{{ u.reputationScore }}</span
                >
              </td>
              <td class="px-4 py-3">
                <span
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  <select
                    ...
                    class="px-2 py-1 border border-gray-200 rounded-lg text-xs bg-white focus:outline-none focus:ring-1 focus:ring-khilafat-400"
                  >
...
const transactions = computed(() =>
  allTransactions.value.length ? allTransactions.value : store.transactions,
);
...
async function fetchTransactions(p = txPage.value) {
  ...
  try {
    ...
    const res = await axios.get("/api/state/transactions", {
      headers: authHeader(),
      params,
    });
...
function toGrams(amount) {
  try {
    return (BigInt(amount) / 1000n).toString();
  } catch {
...
function txBadge(type) {
  return txBadgeMap[type] || "bg-gray-100 text-gray-600";
}
...
async function assignRole(user) {
  ...
  try {
    ...
    roleMsg.value = {
      ok: false,
      text: e.response?.data?.error || "Failed to assign role",
    };
  }
...
function buildCharts() {
  const labels = store.baitulHistory.map((h) =>
  ...
  const totalUserGold = allUsers.value.reduce((sum, u) => {
    try {
      return sum + Number(BigInt(u.goldBalance || "0"));
    } catch {
      return sum;
    }
  }, 0);
  ...
  if (pieChart.value) {
    ...
    pieChartInstance = new Chart(pieChart.value.getContext("2d"), {
      ...
      data: {
        ...
        datasets: [
          {
            data: [totalUserGold, baitul],
            backgroundColor: ["#257a36", "#f6a400"],
          },
        ],
      },
      options: {
...
onMounted(async () => {
  try {
    const usersRes = await axios.get("/api/user/all", {
      headers: authHeader(),
    });
    allUsers.value = usersRes.data;
    allUsers.value.forEach((u) => {
      roleSelections.value[u.id] = u.role;
    });
